...
class Post {
 ...
 public function __construct() {
  ...
  function showAllPost($id) {
   ...
   $mysqli = $db->getConnection();
  	...
  	// prepare and bind
  	$stmt = $mysqli->prepare("SELECT title, content, author, date FROM post WHERE User_id= $id");
    $stmt->bind_result($title, $content, $author, $date_post);
    $stmt->execute();

    while(mysqli_stmt_fetch($stmt)) {

      echo '<div class="allStories">';
      echo '<a class="btn btn-danger btn-sm" href="posts.php?delete=true&userid='.$id.'"><i class="fa fa-trash" aria-hidden="true"></i></a><div class="title"><h1>' .$title. '</h1><h6>'.$date_post. '</h6></div><div id="content"><p>' .$content. '</p></div> <h3>' .$author. '</h3>';
      echo '</div>';
 ...
 public function deletePost($id) {
   ...
   // prepare and bind
   $stmt = $mysqli->prepare("DELETE FROM post WHERE User_id=$id");
